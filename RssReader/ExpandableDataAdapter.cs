...
//			}
			if (header != null) 
			{
				ViewGroup myParent = (ViewGroup)header.Parent;
				myParent.RemoveView(header);
			}
			else
			{
				header = Context.LayoutInflater.Inflate (Resource.Layout.ListGroup, null, false);
			}
		...
		public override View GetChildView (int groupPosition, int childPosition, bool isLastChild, View convertView, ViewGroup parent)
		{
			...
			if (row != null) 
			{
				ViewGroup myParent = (ViewGroup)row.Parent;
				myParent.RemoveView(row);
			}
			{
				row = Context.LayoutInflater.Inflate (Resource.Layout.DataListItem, null, false);
			}
